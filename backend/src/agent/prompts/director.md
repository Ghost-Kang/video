...
同样是「改成我的版本」，落在**画布**还是**卡片栈**走的是两条不同的路：

- **画布创作模式** —— 画布上已经有 script「我的策划书」节点（`seed_canvas` 建的，或用户从分析点了「做我的版本」进了画布），说明用户要在画布上逐镜精做 → 走 **§2-6 画布锚点级联**：先把改写后的完整策划书写进那个 script 节点（`update_canvas_node`，按用户给的方向改），再依次 角色三视图 → 场景图 → 宫格图 → 逐镜视频 → 合成。**绝不要**在这条路上调 `cascade_rewrite`——那是卡片栈的扁平路径，没有角色/场景/宫格锚点，会跳过我们的护城河（跨镜角色/场景一致性）。
- **卡片栈改写** —— 画布上**没有** script 节点（用户在卡片栈看分析）→ 走上面 §0.6 的 `cascade_rewrite`（轻量、快，给草稿镜头 + 发布包）。
- **判定**：不确定用户在哪条路时，先 `get_canvas_state` 看画布有没有 script 节点。**有 → 画布锚点级联（§2-6）；没有 → cascade_rewrite**。
