...
| **Concierge-FirstRun-Agent** | 1h first-run together 陪跑 — creator 跑 Cascade 改写流水线 + 三反馈点收集 | concierge_onboarding_script §3 完整脚本 | on-call(creator commit first-run) | `founder_log/concierge_run_<date>_<creator>.md`(5 列观察表 + 3 反馈点逐字)+ `interview_logged` event | **`Cascade Concierge`**(自建,2026-05-24 W3D4 — `docs/nexus/agents/cascade-concierge.md`,装到 `~/.claude/agents/cascade-concierge.md`)— 深 Cascade 产品流程 + 中文宝妈/育儿语言风格 + 5列观察表/3反馈点的严格纪律 + 不允许使用产品术语;**取代** 原 `customer-service`(太通用,无产品深度)|
...
### 1.2 Phase 1+ supplementary agents(founder W3D4 加,部分可能 Phase 1 即启)

| # | Cascade slot / 用途 | Agent file | Phase | 触发条件 |
|---|---|---|---|---|
| 9 | **Niche-Trend-Researcher**(每周 niche 热点 → 反向输入 Cascade rewrite prompt 迭代) | `product-trend-researcher` | **Phase 1 即启**(W5+ 周节奏)| weekly scheduled |
| 10 | **Cross-Agent-Orchestrator**(PM 跨 agent 协调,若 PM 自己不够用)| `specialized/agents-orchestrator` | Phase 1 备用 | on-call(PM 检测复杂多 agent 任务时) |
| 11 | **WeChat-PrivateDomain-Agent**(creator 长留存 + 1 对 1 私域运营) | `marketing-private-domain-operator` | **W5+ 必上**(Phase 1 D+7 之后接续) | scheduled D+7 起 |
| 12 | **Cultural-Intelligence-Agent**(跨地区文化差异 — baomam 北方 vs 广东 vs 港澳台)| `specialized/specialized-cultural-intelligence-strategist` | Phase 2(cohort 扩到 ≥ 5 人,出现地域分布)| on-call(检测 creator 地域 + 改写命中失败时) |

### 1.2.1 其他 Phase 2+ 候选(暂未启)

| Cascade 未来需求 | Agent file |
|---|---|
| Douyin niche 扩展(辅食外 niche)| `marketing-douyin-strategist` |
| 知乎 / B站 / 微博 跨平台分发 | `marketing-zhihu-strategist` + `bilibili-content-strategist` + `weibo-strategist` |
| creator retention nudges(D+14/+30 拉回)| `product-behavioral-nudge-engine` |
