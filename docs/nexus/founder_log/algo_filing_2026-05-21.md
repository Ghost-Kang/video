...
- [ ] **营业执照副本扫描件**: **在办中**,预计 `<FILL 预计取证月份,如 2026-06>` 取证;取证后扫描件放于 `algo_filing_attachments/01_business_license.pdf`(本地敏感目录,gitignored)
  - **提交备案时的临时口径**:在备案系统"主体类型"选项中,若有"个人/独资"选项则使用;否则在备注栏说明"主体在办中,以法人个人责任先行启动备案,营业执照取证后补充上传"
- [ ] **法人身份证扫描件**: 正反两面,放于 `algo_filing_attachments/02_legal_person_id_*.jpg`(本地敏感目录,gitignored)— 提交备案时上传系统;**绝不 commit 进 git**
- [ ] **域名 ICP 备案号**: Phase 1 内测期间使用 founder 自有域名 `<FILL 你的域名,如 openrhtv.example.com>` 进行**邀请制访问**(非公网商业服务);该域名 ICP 备案预计 Phase 2 公测前完成;**当前阶段在备案系统"是否取得 ICP 备案"处选"否"并在备注栏说明:"Phase 1 仅邀请制内测,不上公网商业服务;Phase 2 公测前完成 ICP 备案"**
- [ ] **服务器境内托管证明**: Phase 1 当前阶段为 **本地开发环境**(macOS dev 机)运行,无云端部署;Phase 2 公测前迁移至阿里云/腾讯云境内区(预计 `<FILL Phase 2 迁移月份>`)。备案系统"服务器物理位置"处填"本地开发环境,Phase 2 公测前迁至境内云"
