...
当前状态:**dormant / superseded**;`scripts/p4-9_toprador_staging.py` 现在默认走 `CASCADE_UPSTREAM=mediakit` 的豆包/ARK 模型链路,仅在显式 `--upstream toprador` 时才需要旧 Toprador HTTP endpoint。

---

**Owner**: Codex session (backend) — superseded by P5-3;不再 blocked on Toprador endpoint/key
...
**Status (original)**: DRAFT · old Toprador HTTP path would need `TOPRADOR_ENDPOINT`;current staging default uses MediaKit/Doubao path
**Time budget**: 0.5 day
...
旧 Toprador HTTP path only: founder 在 `backend/.env` 加:
```
TOPRADOR_ENDPOINT=https://<toprador 实际 base url>/analyze
TOPRADOR_API_KEY=<token>  # optional if endpoint requires auth
```

当前默认 staging 不需要上述两项;使用 `.env` 里的 `CASCADE_UPSTREAM=mediakit`, `ARK_API_KEY`, `DOUBAO_MODEL`, `VOLC_MEDIAKIT_AK`。
...
- ✅ 不再等待 `TOPRADOR_ENDPOINT` + `TOPRADOR_API_KEY`;默认用 MediaKit/Doubao path
