...
>
> **2026-06-02 再更新:apimart 阻断已解除** —— 草稿图改走**火山 Seedream**(复用现有
> ARK key,无需新密钥),prod 实测全链跑通(改写→草稿图→图生视频→合成→发布,一个 ARK key)。
>
> **剩余 TODO(再更新)**:
> 1. ✅【已解除】草稿图 key —— 改 Seedream 用 ARK key,**不再需要任何新生图密钥**。
