    ...
    <PageShell>
      <main className="px-6 pt-16 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <section className="rounded-2xl bg-white dark:bg-stone-900 border border-stone-200/70 dark:border-stone-800/70 p-5 shadow-soft">
            <h2 className="text-sm uppercase tracking-wider text-stone-500 dark:text-stone-400 mb-3">按 creator</h2>
            {by_user.length === 0 ? (
              isLoading ? <p className="text-sm text-stone-500 dark:text-stone-400">加载中…</p> : <EmptyHint />
            ) : (
                  ...
                  {by_user.slice(0, 10).map((row) => {
                    const tone =
                      row.is_runaway_red
                        ...
                        : "";
                    return (
                      <tr key={row.user_id} className={`border-t border-stone-100 dark:border-stone-800 ${tone}`}>
                        ...
                        <td className="px-2 py-2 text-right tabular-nums text-stone-500 dark:text-stone-400">{row.calls}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <section className="rounded-2xl bg-white dark:bg-stone-900 border border-stone-200/70 dark:border-stone-800/70 p-5 shadow-soft">
            <h2 className="text-sm uppercase tracking-wider text-stone-500 dark:text-stone-400 mb-3">按 call_kind</h2>
            {by_kind.length === 0 ? (
              isLoading ? <p className="text-sm text-stone-500 dark:text-stone-400">加载中…</p> : <EmptyHint />
            ) : (
                ...
                {by_kind.map((row) => (
                  <li key={row.call_kind}>
                    <div className="flex items-center justify-between gap-2 text-sm">
                      <span className="font-mono text-xs text-stone-700 dark:text-stone-300">{row.call_kind}</span>
                      <span className="text-stone-500 dark:text-stone-400 tabular-nums">
                        {fmtCny(row.total_cny)} · {fmtShare(row.share)} · {row.calls} 次
                      </span>
                    </div>
                    <div className="mt-1 h-2 rounded-full bg-stone-100 dark:bg-stone-800 overflow-hidden">
                      <div
...
function TrendChart({ points }: TrendChartProps) {
  ...
  return (
    <div className="overflow-x-auto text-stone-900 dark:text-stone-100" role="img" aria-label="14 日成本 trend">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-32" preserveAspectRatio="none">
        <path d={pathD} fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinejoin="round" />
        {points.map((p, i) => {
          const x = padX + i * stepX;
          const y = padY + innerH - (max > 0 ? (p.cny / max) * innerH : 0);
          return <circle key={p.date} cx={x} cy={y} r={2} fill="currentColor" />;
        })}
        <text x={padX} y={H - 1} fontSize="8" fill="currentColor" opacity="0.55">
          {points[0]?.date}
        </text>
        <text x={W - padX} y={H - 1} fontSize="8" fill="currentColor" opacity="0.55" textAnchor="end">
          {points[points.length - 1]?.date}
        </text>
        <text x={padX} y={padY + 6} fontSize="8" fill="currentColor" opacity="0.55">
          max ¥{max.toFixed(2)}
        </text>
      </svg>
    </div>
